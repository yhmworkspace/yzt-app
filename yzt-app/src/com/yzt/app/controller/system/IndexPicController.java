...
public class IndexPicController extends BaseController {
	...
	public String index( ModelMap m){
		return "system/indexpic/indexpiclist";
	}
	
	
	@RequestMapping("/listjson")
	@ResponseBody
	public EasyuiDataGridJson listjson(YztIndexPic pic,EasyuiDataGrid dg, ModelMap m){
		return indexPicService.datagrid(pic, dg);
	}
	
	@RequestMapping("add")
	public String add( ModelMap m){
		return "system/indexpic/indexpicinfo";
	}
	...
	@RequestMapping("/save")
	@ResponseBody
	public Json save(YztIndexPic logo,@RequestParam("imgFile") MultipartFile file,HttpServletRequest request,ModelMap m) throws ParseException, IllegalStateException, IOException{
		Json json = new Json();
		...
		if(logo.getId() !=null && !logo.getId().isEmpty()){//修改
			if(!file.isEmpty()){
				...
				String realRootPath = getServletContext().getResource("/").toString()+"upload/indexpic/";
				...
				String logurl = basePath+"upload/indexpic/"+logo.getId()+"."+filetype;
				logo.setLogoUrl(logurl);
			}
			//commodity.setStatus(0);
			int flag = indexPicService.updateNotNull(logo);
			if(flag>0){
				json.setMsg("修改成功");
			...
			if(!file.isEmpty()){
				...
				String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
				//System.out.println("getContextPath==="+ basePath+"upload/user/headimg/"+user.getId()+"."+filetype);
				String picurl = basePath+"upload/indexpic/"+logo.getId()+"."+filetype;
				logo.setLogoUrl(picurl);
			}
			logo.setStatus(0);
			int flag = indexPicService.save(logo);
			if(flag>0){
				json.setMsg("保存成功");
				json.setSuccess(true);
				//json.setObj(learningcenter);
	...
	public Json updateStatus(@PathVariable int status,String ids,ModelMap m) throws Exception{
		...
		if(0 == status){
   		...
   		String[] id = ids.split(",");
   		for(int i =0;i<id.length;i++){
   			indexPicService.updateBySql(" set status="+num+" where id='"+id[i]+"'");
   		}
   		json.setSuccess(true);
   		json.setMsg("设置成功");
   		return json;
	...
	public Json setweight(int is_weight,String ids,ModelMap m) throws Exception{
		 Json json = new Json();
   		...
   		String[] id = ids.split(",");
   		for(int i =0;i<id.length;i++){
   			indexPicService.updateBySql(" set weight="+is_weight+" where id='"+id[i]+"'");
   		}
   		json.setSuccess(true);
   		json.setMsg("设置成功");
   		return json;
